...
class SB3_MAS_Train_Parallelized_Threads:
    ...
    def get_action_single_thread(self, agent_id, obs):
        if(np.random.random() < self.eps):
            action = self.models[agent_id].action_space.sample()
        else:
            action, _ = self.models[agent_id].predict(obs, deterministic=False)
    ...
    def get_all_action_parallel(self, observations):
        actions_encoded = {}
        actions = {}
        
        futures = [
            self.executor.submit(self.get_action_single_thread, agent_id, observations[agent_id])
            for agent_id in range(self.num_agents)
        ]

        for future in futures:
            agent_id, action_encoded = future.result()
            actions_encoded[agent_id] = action_encoded
            actions[agent_id] = self.decode(action_encoded)
            
        return actions_encoded, actions
    ...
    def train(self):
        ...
        for i in range(0, self.num_episodes):
            ...
            while self.env.agents:
                ...
                actions_encoded, actions = self.get_all_action_parallel(obs)
    ...
    # print(rewards)
        ...
        plt.tight_layout()
        plt.savefig(f"rewards_plot_{self.num_episodes - 1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.num_agents}agents_threads_{self.mode}.pdf")
        plt.close()
    ...
    def plot_battery_levels(self, levels):
        ...
        plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
        plt.tight_layout()
        plt.savefig(f"avg_battery_plot_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.num_agents}agents_threads_{self.mode}.pdf")
        plt.close()
    ...
    def plot_battery_levels_individual(self, levels):
        ...
        for i in range(0, self.env._num_agents):
            ...
            plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
            plt.tight_layout()
            plt.savefig(f"avg_battery_plot_{self.battery_capacities[i]}Wh_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.num_agents}agents_threads_{self.mode}.pdf")
            plt.close()
    ...
    def plot_backlogs(self, backlogs):
        ...
        plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
        plt.tight_layout()
        plt.savefig(f"backlog_plot_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.num_agents}agents_threads_{self.mode}.pdf")
        plt.close()
    ...
    def plot_battery_daily(self, data):    
        for elem in range(0, self.env._num_agents):
            ...
            plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
            plt.tight_layout()
            plt.savefig(f"battery_{int(self.env.battery_capacities[elem] / 3600)}Wh_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.num_agents}agents_threads_{self.mode}.pdf")
            plt.close()
    ...
    def plot_backlog_daily(self, data):
        ...
        for elem in range(0, self.env._num_agents):
            ...
            plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
            plt.tight_layout()
            plt.savefig(f"backlog_{int(self.env.battery_capacities[elem] / 3600)}Wh_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.num_agents}agents_threads_{self.mode}.pdf")
            plt.close()        
    ...
    def plot_framerate(self, fs):
        ...
        plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
        plt.tight_layout()
        plt.savefig(f"framerate_plot_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.env._num_agents}agents_threads_{self.mode}.pdf")
        plt.close()
    ...
    def plot_local_framerate(self, fs):
        ...
        plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
        plt.tight_layout()
        plt.savefig(f"local_framerate_plot_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.env._num_agents}agents_threads_{self.mode}.pdf")
        plt.close()
    ...
    def plot_offloading_framerate(self, fs):
        ...
        plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
        plt.tight_layout()
        plt.savefig(f"offloading_framerate_plot_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.env._num_agents}agents_threads_{self.mode}.pdf")
        plt.close()
    ...
    def plot_offloading_matchings(self, fs):
        ...
        plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
        plt.tight_layout()
        plt.savefig(f"offloading_matchings_plot_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.env._num_agents}agents_threads_{self.mode}.pdf")
        plt.close()

    def save_battery_csv(self, battery_daily):
        
        for agent_id in range(self.num_agents):
            filename = f"./csvs/csvs_batch_{self.batch_size}/parallelized_battery_{self.battery_capacities[agent_id]}_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.num_agents}agents_threads_{self.mode}.csv"
    ...
    def save_backlog_csv(self, backlog_daily):
        ...
        for agent_id in range(self.num_agents):
            filename = f"./csvs/csvs_batch_{self.batch_size}/parallelized_backlog_{self.battery_capacities[agent_id]}_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.num_agents}agents_threads_{self.mode}.csv"
    ...
    def save_rewards_csv(self, rewards):
        ...
        for agent_id in range(self.num_agents):
            filename = f"./csvs/csvs_batch_{self.batch_size}/parallelized_rewards_agent_{self.battery_capacities[agent_id]}_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.num_agents}agents_threads_{self.mode}.csv"
    ...
    def save_time_csv(self, times):
        os.makedirs('./csvs/csvs_batch_{self.batch_size}', exist_ok=True)
        filename = f"./csvs/csvs_batch_{self.batch_size}/parallelized_time_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.num_agents}agents_threads_{self.mode}.csv"
